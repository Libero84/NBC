...
.directive('ibanNumber', function () {
  return {
    require: 'ngModel',
    link: function (scope, element, attributes, control) {
      control.$validators.ibanNumber = function (modelValue, viewValue) {
        if (control.$isEmpty(modelValue)) {
          return true;
        }

        var ibanValue = viewValue,
          codeLengths = {
          ...
          iban = ibanValue.toUpperCase().replace(/[^A-Z0-9]/g, ''),
      ...
      function mod97(digit) {
        var checksum = digit.slice(0, 2), fragment;
        for (var offset = 2; offset < digit.length; offset += 7) {
          fragment = String(checksum) + digit.substring(offset, offset + 7);
